...
describe('TextField Component', () => {
    ...
    it('should apply the mask correctly', () => {
        const { getByLabelText } = render(<TextField label="CPF" id="cpf" mask={cpfMask} />);
    ...
    it('should limit the input length', () => {
        const { getByLabelText } = render(<TextField label="CPF" id="cpf" maxLength={14} />);
        const input = getByLabelText('CPF') as HTMLInputElement;
    ...
    it('should display error message', () => {
        const { getByText } = render(<TextField label="CPF" id="cpf" error="Invalid CPF" />);
        expect(getByText('Invalid CPF')).toBeInTheDocument();
    });

    it('should call onChange handler', () => {
        const handleChange = jest.fn();
        const { getByLabelText } = render(<TextField label="CPF" id="cpf" onChange={handleChange} />);
        const input = getByLabelText('CPF') as HTMLInputElement;
